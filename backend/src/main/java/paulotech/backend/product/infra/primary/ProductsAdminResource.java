...
import paulotech.backend.product.infra.primary.exceptions.MultipartPictureException;
...
public class ProductsAdminResource {
    ...
    private Function<MultipartFile, RestPicture> mapMultipartFileToRestPicture(){
        return multipartFile -> {
            try {
                ...
                throw new MultipartPictureException(String.format("Cannot parse multipart file : %s", multipartFile));
            }
        };
    }
    ...
    public ResponseEntity<UUID> delete(@RequestParam("publicId") UUID id){
        try {
            PublicId deletedProductUUID = productApplicationService.deleteProduct(new PublicId(id));
            return ResponseEntity.ok(deletedProductUUID.value());
        } catch (EntityNotFoundException enfee){
            log.error("Cannot delete product with id : {}", id, enfee);
    ...
    public ResponseEntity<Page<RestProduct>> getAll(Pageable pageable){
        Page<Product> products = productApplicationService.findAllProduct(pageable);
